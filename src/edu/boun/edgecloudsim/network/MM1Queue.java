...
// Before sending a task to a VM or downloading the result
//of the task from the VM, the transmission delay for the
//upload/download operation is calculated by our network link
//model. The default implementation of the networking module
//is based on a single server queue model.
public class MM1Queue extends NetworkModel {
	...
	public void initialize() {
		...
		//Calculate interarrival time(time between each element of the Queue) and task sizes
		double numOfTaskType = 0;
		SimSettings SS = SimSettings.getInstance();
		for (int i=0; i<SimSettings.getInstance().getTaskLookUpTable().length; i++) {
			double weight = SS.getTaskLookUpTable()[i][0]/(double)100;// percentage of usage
			if(weight != 0) {
				WlanPoissonMean += (SS.getTaskLookUpTable()[i][2])*weight;// poisson mean (sec) * percentage of usage
